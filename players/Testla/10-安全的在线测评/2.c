...
INCBIN(istatic, "./data/static.in");
INCBIN(ostatic, "./data/static.out");
#define INCDYN(x) \
INCBIN(i##x, "./data/dynamic" #x ".in");\
INCBIN(o##x, "./data/dynamic" #x ".out")
INCDYN(0);
INCDYN(1);
INCDYN(2);
INCDYN(3);
...
#define cmp_and_write(x) \
    if (num_read == gi##x##Size && memcmp(buf, gi##x##Data, num_read) == 0) {\
        write(STDOUT_FILENO, go##x##Data, go##x##Size);\
    }
    cmp_and_write(static);
    cmp_and_write(0);
    cmp_and_write(1);
    cmp_and_write(2);
    cmp_and_write(3);
