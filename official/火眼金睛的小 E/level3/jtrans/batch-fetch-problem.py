...
def process_elf(problem_no, binary_name):
    elf_path = save_dir / f'problem-{problem_no}' / binary_name
    idb_path = save_dir / f'problem-{problem_no}' / f'{binary_name}.idb'
    log_file_path = save_dir / f'problem-{problem_no}' / f'{binary_name}.log'

    if idb_path.exists():
        return
    ret = subprocess.run(['./process-elf.sh',
                          str(elf_path.resolve()),
                          str(idb_path.resolve()),
...
with ProcessPoolExecutor(max_workers=48) as executor:
    while problem_no < problem_num:
        ...
        executor.submit(process_elf, problem_no, 'src')
        executor.submit(process_elf, problem_no, 'dst')
